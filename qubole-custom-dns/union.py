...
CNAME_DOMAIN_SUFFIX = 'qubole-dns.net'
...
def lambda_handler(event, context):
    ...
    if state == 'running':
        instance = compute.describe_instances(InstanceIds=[instance_id])  # Remove response metadata from the response
        instance.pop('ResponseMetadata')
        # Remove null values from the response. You cannot save a dict/JSON document in DynamoDB if it
    ...
    for tag in tags:
        if 'QUBOLE' in tag.get('Key', {}).lstrip().upper():
            ...
            for cname_private_hosted_zone in private_hosted_zone_collection:
                ...
                if cname_domain_suffix_id == cname_private_hosted_zone_id:  # create CNAME record in private zone
                    if state == 'running':
                        try:
                            create_resource_record(cname_private_hosted_zone_id, cname_host_name,
                                                   cname_private_hosted_zone,
                                                   'CNAME', private_dns_name)
                        except BaseException as e:
                            print e
                    else:
                        try:
                            delete_resource_record(cname_private_hosted_zone_id, cname_host_name,
                                                   cname_private_hosted_zone,
                                                   'CNAME', private_dns_name)
                        except BaseException as e:
                            print e
            ...
            try:
                ...
                exit()
            # Look to see whether there's a DHCP option set assigned to the VPC.  If there is, use the value of
            ...
            for domain in domains:
                ...
                if domain in private_hosted_zone_collection:
                    ...
                    print 'No matching zone for %s' % domain
