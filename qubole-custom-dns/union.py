            ...
            print 'Associating zone %s with VPC %s' % (private_hosted_zone_id, vpc_id)
                try:
                  ...
                  print 'You cannot create an association with a VPC with an overlapping subdomain.\n', e
                    exit()
          try:
            create_resource_record(private_hosted_zone_id, cname_host_name, private_hosted_zone_name, 'A', private_ip)
            create_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', ".".join([cname_host_name, private_hosted_zone_name]))
          except BaseException as e:
            print e
        else:
          try:
            delete_resource_record(private_hosted_zone_id, cname_host_name, private_hosted_zone_name, 'A', private_ip)
            delete_resource_record(reverse_lookup_zone_id, reversed_ip_address, 'in-addr.arpa', 'PTR', ".".join([cname_hostname, private_hosted_zone_name]))
          except BaseException as e:
            print e
      else:
